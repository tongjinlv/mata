 ...
 563                     ; 122     BaudRate_Mantissa100 = (((uint32_t)CLK_GetClockFreq() * 100) / (BaudRate << 4));
 ...
 585                     ; 124     UART1->BRR2 |= (uint8_t)((uint8_t)(((BaudRate_Mantissa100 - (BaudRate_Mantissa * 100)) << 4) / 100) & (uint8_t)0x0F); 
 ...
 616                     ; 126     UART1->BRR2 |= (uint8_t)((BaudRate_Mantissa >> 4) & (uint8_t)0xF0); 
 ...
 773                     ; 177 void UART1_Cmd(FunctionalState NewState)
 773                     ; 178 {
 774                     .text:	section	.text,new
 775  0000               _UART1_Cmd:
 779                     ; 179     if (NewState != DISABLE)
 781  0000 4d            	tnz	a
 782  0001 2705          	jreq	L732
 783                     ; 182         UART1->CR1 &= (uint8_t)(~UART1_CR1_UARTD); 
 785  0003 721b5234      	bres	21044,#5
 788  0007 81            	ret	
 789  0008               L732:
 790                     ; 187         UART1->CR1 |= UART1_CR1_UARTD;  
 792  0008 721a5234      	bset	21044,#5
 793                     ; 189 }
 796  000c 81            	ret	
 922                     ; 205 void UART1_ITConfig(UART1_IT_TypeDef UART1_IT, FunctionalState NewState)
 922                     ; 206 {
 ...
 931                     ; 207     uint8_t uartreg = 0, itpos = 0x00;
 935                     ; 210     assert_param(IS_UART1_CONFIG_IT_OK(UART1_IT));
 ...
 949  0020 ae00d2        	ldw	x,#210
 950  0023 ad70          	call	LC006
 951  0025               L401:
 952                     ; 211     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
 954  0025 7b07          	ld	a,(OFST+5,sp)
 955  0027 2708          	jreq	L411
 956  0029 4a            	dec	a
 957  002a 2705          	jreq	L411
 958  002c ae00d3        	ldw	x,#211
 959  002f ad64          	call	LC006
 960  0031               L411:
 961                     ; 214     uartreg = (uint8_t)((uint16_t)UART1_IT >> 0x08);
 963  0031 7b03          	ld	a,(OFST+1,sp)
 964  0033 6b01          	ld	(OFST-1,sp),a
 965                     ; 216     itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
1027  007a               L533:
1028                     ; 241         else if (uartreg == 0x02)
1030  007a a102          	cp	a,#2
1031  007c 260b          	jrne	L143
1032                     ; 243             UART1->CR2 &= (uint8_t)(~itpos);
...
1041  0089               L143:
1042                     ; 247             UART1->CR4 &= (uint8_t)(~itpos);
...
1063  00a0 81            	ret	
1100                     ; 258 void UART1_HalfDuplexCmd(FunctionalState NewState)
1100                     ; 259 {
1101                     .text:	section	.text,new
1102  0000               _UART1_HalfDuplexCmd:
1104  0000 88            	push	a
1105       00000000      OFST:	set	0
1108                     ; 260     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
1110  0001 4d            	tnz	a
1111  0002 2711          	jreq	L231
1112  0004 4a            	dec	a
1113  0005 270e          	jreq	L231
1114  0007 ae0104        	ldw	x,#260
...
1122  0015               L231:
1123                     ; 262     if (NewState != DISABLE)
1125  0015 7b01          	ld	a,(OFST+1,sp)
1126  0017 2706          	jreq	L363
1127                     ; 264         UART1->CR5 |= UART1_CR5_HDSEL;  /**< UART1 Half Duplex Enable  */
1129  0019 72165238      	bset	21048,#3
1131  001d 2004          	jra	L563
1132  001f               L363:
1133                     ; 268         UART1->CR5 &= (uint8_t)~UART1_CR5_HDSEL; /**< UART1 Half Duplex Disable */
1135  001f 72175238      	bres	21048,#3
1136  0023               L563:
1137                     ; 270 }
1140  0023 84            	pop	a
1141  0024 81            	ret	
1199                     ; 278 void UART1_IrDAConfig(UART1_IrDAMode_TypeDef UART1_IrDAMode)
1199                     ; 279 {
1200                     .text:	section	.text,new
1201  0000               _UART1_IrDAConfig:
1203  0000 88            	push	a
1204       00000000      OFST:	set	0
1207                     ; 280     assert_param(IS_UART1_IRDAMODE_OK(UART1_IrDAMode));
1209  0001 a101          	cp	a,#1
1210  0003 2711          	jreq	L441
1211  0005 4d            	tnz	a
1212  0006 270e          	jreq	L441
1213  0008 ae0118        	ldw	x,#280
...
1298  0015               L651:
1299                     ; 304     if (NewState != DISABLE)
1301  0015 7b01          	ld	a,(OFST+1,sp)
1302  0017 2706          	jreq	L734
1303                     ; 307         UART1->CR5 |= UART1_CR5_IREN;
1305  0019 72125238      	bset	21048,#1
1307  001d 2004          	jra	L144
1308  001f               L734:
1309                     ; 312         UART1->CR5 &= ((uint8_t)~UART1_CR5_IREN);
1311  001f 72135238      	bres	21048,#1
1312  0023               L144:
1313                     ; 314 }
1316  0023 84            	pop	a
1317  0024 81            	ret	
1377                     ; 323 void UART1_LINBreakDetectionConfig(UART1_LINBreakDetectionLength_TypeDef UART1_LINBreakDetectionLength)
1377                     ; 324 {
1378                     .text:	section	.text,new
1379  0000               _UART1_LINBreakDetectionConfig:
1381  0000 88            	push	a
1382       00000000      OFST:	set	0
1385                     ; 325     assert_param(IS_UART1_LINBREAKDETECTIONLENGTH_OK(UART1_LINBreakDetectionLength));
1387  0001 4d            	tnz	a
1388  0002 2711          	jreq	L071
1389  0004 4a            	dec	a
1390  0005 270e          	jreq	L071
1391  0007 ae0145        	ldw	x,#325
...
1399  0015               L071:
1400                     ; 327     if (UART1_LINBreakDetectionLength != UART1_LINBREAKDETECTIONLENGTH_10BITS)
1402  0015 7b01          	ld	a,(OFST+1,sp)
1403  0017 2706          	jreq	L174
1404                     ; 329         UART1->CR4 |= UART1_CR4_LBDL;
1406  0019 721a5237      	bset	21047,#5
1408  001d 2004          	jra	L374
1409  001f               L174:
1410                     ; 333         UART1->CR4 &= ((uint8_t)~UART1_CR4_LBDL);
1412  001f 721b5237      	bres	21047,#5
1413  0023               L374:
1414                     ; 335 }
1417  0023 84            	pop	a
1418  0024 81            	ret	
1454                     ; 343 void UART1_LINCmd(FunctionalState NewState)
1454                     ; 344 {
1455                     .text:	section	.text,new
1456  0000               _UART1_LINCmd:
1458  0000 88            	push	a
1459       00000000      OFST:	set	0
1462                     ; 345     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
1464  0001 4d            	tnz	a
1465  0002 2711          	jreq	L202
1466  0004 4a            	dec	a
1467  0005 270e          	jreq	L202
1468  0007 ae0159        	ldw	x,#345
...
1730  0016               L042:
1731                     ; 413     UART1->CR1 &= ((uint8_t)~UART1_CR1_WAKE);
1733  0016 72175234      	bres	21044,#3
1734                     ; 414     UART1->CR1 |= (uint8_t)UART1_WakeUp;
...
1742  0022 84            	pop	a
1743  0023 81            	ret	
1780                     ; 422 void UART1_ReceiverWakeUpCmd(FunctionalState NewState)
1780                     ; 423 {
1781                     .text:	section	.text,new
1782  0000               _UART1_ReceiverWakeUpCmd:
1784  0000 88            	push	a
1785       00000000      OFST:	set	0
1788                     ; 424     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
1790  0001 4d            	tnz	a
1791  0002 2711          	jreq	L252
1792  0004 4a            	dec	a
1793  0005 270e          	jreq	L252
1794  0007 ae01a8        	ldw	x,#424
...
1907                     ; 458   return (uint16_t)( (((uint16_t) UART1->DR) | temp ) & ((uint16_t)0x01FF));
1909  000b 5f            	clrw	x
...
2132  0023 84            	pop	a
2133  0024 81            	ret	
2167                     ; 520 void UART1_SetGuardTime(uint8_t UART1_GuardTime)
2167                     ; 521 {
2168                     .text:	section	.text,new
2169  0000               _UART1_SetGuardTime:
2173                     ; 523     UART1->GTR = UART1_GuardTime;
2175  0000 c75239        	ld	21049,a
2176                     ; 524 }
2179  0003 81            	ret	
2213                     ; 548 void UART1_SetPrescaler(uint8_t UART1_Prescaler)
2213                     ; 549 {
2214                     .text:	section	.text,new
2215  0000               _UART1_SetPrescaler:
2219                     ; 551     UART1->PSCR = UART1_Prescaler;
2221  0000 c7523a        	ld	21050,a
2222                     ; 552 }
2225  0003 81            	ret	
2369                     ; 560 FlagStatus UART1_GetFlagStatus(UART1_Flag_TypeDef UART1_FLAG)
2369                     ; 561 {
...
2378                     ; 562     FlagStatus status = RESET;
2380                     ; 565     assert_param(IS_UART1_FLAG_OK(UART1_FLAG));
...
2410  0042               L213:
2411                     ; 569     if (UART1_FLAG == UART1_FLAG_LBDF)
2413  0042 1e02          	ldw	x,(OFST+1,sp)
2414  0044 a30210        	cpw	x,#528
2415  0047 2605          	jrne	L1601
2416                     ; 571         if ((UART1->CR4 & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
2418  0049 c65237        	ld	a,21047
2419                     ; 574             status = SET;
2421  004c 2008          	jp	LC009
2422                     ; 579             status = RESET;
2423  004e               L1601:
2424                     ; 582     else if (UART1_FLAG == UART1_FLAG_SBK)
2426  004e a30101        	cpw	x,#257
2427  0051 260b          	jrne	L1701
2428                     ; 584         if ((UART1->CR2 & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
...
2521  0019               L423:
2522                     ; 644     if (UART1_FLAG == UART1_FLAG_RXNE)
2524  0019 1e01          	ldw	x,(OFST+1,sp)
2525  001b a30020        	cpw	x,#32
2526  001e 2606          	jrne	L3211
2527                     ; 646         UART1->SR = (uint8_t)~(UART1_SR_RXNE);
2529  0020 35df5230      	mov	21040,#223
2531  0024 2004          	jra	L5211
2532  0026               L3211:
2533                     ; 651         UART1->CR4 &= (uint8_t)~(UART1_CR4_LBDF);
2535  0026 72195237      	bres	21047,#4
2536  002a               L5211:
2537                     ; 653 }
2540  002a 85            	popw	x
2541  002b 81            	ret	
2624                     ; 668 ITStatus UART1_GetITStatus(UART1_IT_TypeDef UART1_IT)
2624                     ; 669 {
...
2633                     ; 670     ITStatus pendingbitstatus = RESET;
2635                     ; 671     uint8_t itpos = 0;
2637                     ; 672     uint8_t itmask1 = 0;
2639                     ; 673     uint8_t itmask2 = 0;
2641                     ; 674     uint8_t enablestatus = 0;
2643                     ; 677     assert_param(IS_UART1_GET_IT_OK(UART1_IT));
...
2667  0033               L633:
2668                     ; 680     itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
2683                     ; 682     itmask1 = (uint8_t)((uint8_t)UART1_IT >> (uint8_t)4);
2685  0044 7b04          	ld	a,(OFST+2,sp)
2686  0046 4e            	swap	a
2687  0047 a40f          	and	a,#15
2688  0049 6b02          	ld	(OFST+0,sp),a
2689                     ; 684     itmask2 = (uint8_t)((uint8_t)1 << itmask1);
2691  004b 5f            	clrw	x
