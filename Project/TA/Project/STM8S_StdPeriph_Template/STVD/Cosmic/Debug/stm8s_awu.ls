  ...
  56  0020 0f            	dc.b	15
  57  0021 0f            	dc.b	15
  86                     ; 67 void AWU_DeInit(void)
  86                     ; 68 {
  88                     .text:	section	.text,new
  89  0000               _AWU_DeInit:
  93                     ; 69     AWU->CSR = AWU_CSR_RESET_VALUE;
  95  0000 725f50f0      	clr	20720
  96                     ; 70     AWU->APR = AWU_APR_RESET_VALUE;
  98  0004 353f50f1      	mov	20721,#63
  99                     ; 71     AWU->TBR = AWU_TBR_RESET_VALUE;
 101  0008 725f50f2      	clr	20722
 102                     ; 72 }
 105  000c 81            	ret	
 268                     ; 82 void AWU_Init(AWU_Timebase_TypeDef AWU_TimeBase)
 268                     ; 83 {
 269                     .text:	section	.text,new
 270  0000               _AWU_Init:
 272  0000 88            	push	a
 273       00000000      OFST:	set	0
 276                     ; 86     assert_param(IS_AWU_TIMEBASE_OK(AWU_TimeBase));
 278  0001 4d            	tnz	a
 ...
 320  0052               L41:
 321                     ; 89     AWU->CSR |= AWU_CSR_AWUEN;
 323  0052 721850f0      	bset	20720,#4
 324                     ; 92     AWU->TBR &= (uint8_t)(~AWU_TBR_AWUTB);
 326  0056 c650f2        	ld	a,20722
 327  0059 a4f0          	and	a,#240
 328  005b c750f2        	ld	20722,a
 329                     ; 93     AWU->TBR |= TBR_Array[(uint8_t)AWU_TimeBase];
 331  005e 5f            	clrw	x
 ...
 337                     ; 96     AWU->APR &= (uint8_t)(~AWU_APR_APR);
 339  006b c650f1        	ld	a,20721
 340  006e a4c0          	and	a,#192
 341  0070 c750f1        	ld	20721,a
 342                     ; 97     AWU->APR |= APR_Array[(uint8_t)AWU_TimeBase];
 344  0073 5f            	clrw	x
 ...
 353  0080 84            	pop	a
 354  0081 81            	ret	
 409                     ; 108 void AWU_Cmd(FunctionalState NewState)
 409                     ; 109 {
 410                     .text:	section	.text,new
 411  0000               _AWU_Cmd:
 415                     ; 110     if (NewState != DISABLE)
 417  0000 4d            	tnz	a
 418  0001 2705          	jreq	L531
 419                     ; 113         AWU->CSR |= AWU_CSR_AWUEN;
 421  0003 721850f0      	bset	20720,#4
 424  0007 81            	ret	
 425  0008               L531:
 426                     ; 118         AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 ...
 493                     ; 135 void AWU_LSICalibrationConfig(uint32_t LSIFreqHz)
 493                     ; 136 {
 494                     .text:	section	.text,new
 495  0000               _AWU_LSICalibrationConfig:
 497  0000 5206          	subw	sp,#6
 498       00000006      OFST:	set	6
 501                     ; 138     uint16_t lsifreqkhz = 0x0;
 503                     ; 139     uint16_t A = 0x0;
 505                     ; 142     assert_param(IS_LSI_FREQUENCY_OK(LSIFreqHz));
 ...
 549                     ; 150     if ((4U * A) >= ((lsifreqkhz - (4U * A)) * (1U + (2U * A))))
 551  0043 58            	sllw	x
 552  0044 58            	sllw	x
 ...
 568                     ; 152         AWU->APR = (uint8_t)(A - 2U);
 570  0064 a002          	sub	a,#2
 572  0066 2001          	jra	L171
 573  0068               L761:
 574                     ; 156         AWU->APR = (uint8_t)(A - 1U);
 576  0068 4a            	dec	a
 577  0069               L171:
 ...
 583  006e 81            	ret	
 606                     ; 165 void AWU_IdleModeEnable(void)
 606                     ; 166 {
 607                     .text:	section	.text,new
 608  0000               _AWU_IdleModeEnable:
 612                     ; 168     AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 614  0000 721950f0      	bres	20720,#4
 615                     ; 171     AWU->TBR = (uint8_t)(~AWU_TBR_AWUTB);
 617  0004 35f050f2      	mov	20722,#240
 618                     ; 172 }
 621  0008 81            	ret	
 665                     ; 180 FlagStatus AWU_GetFlagStatus(void)
 665                     ; 181 {
 666                     .text:	section	.text,new
 667  0000               _AWU_GetFlagStatus:
 671                     ; 182     return((FlagStatus)(((uint8_t)(AWU->CSR & AWU_CSR_AWUF) == (uint8_t)0x00) ? RESET : SET));
