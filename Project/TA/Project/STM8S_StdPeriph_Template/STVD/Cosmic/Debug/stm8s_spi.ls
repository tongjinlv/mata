  ...
  49                     ; 44 void SPI_DeInit(void)
  49                     ; 45 {
  51                     .text:	section	.text,new
  52  0000               _SPI_DeInit:
  56                     ; 46     SPI->CR1    = SPI_CR1_RESET_VALUE;
  58  0000 725f5200      	clr	20992
  59                     ; 47     SPI->CR2    = SPI_CR2_RESET_VALUE;
  61  0004 725f5201      	clr	20993
  62                     ; 48     SPI->ICR    = SPI_ICR_RESET_VALUE;
  64  0008 725f5202      	clr	20994
  65                     ; 49     SPI->SR     = SPI_SR_RESET_VALUE;
  67  000c 35025203      	mov	20995,#2
  68                     ; 50     SPI->CRCPR  = SPI_CRCPR_RESET_VALUE;
 ...
 408  000b ae004b        	ldw	x,#75
 409  000e cd00c2        	call	LC001
 410  0011               L41:
 411                     ; 76     assert_param(IS_SPI_BAUDRATE_PRESCALER_OK(BaudRatePrescaler));
 ...
 470  006f ae0050        	ldw	x,#80
 471  0072 ad4e          	call	LC001
 472  0074               L46:
 473                     ; 81     assert_param(IS_SPI_SLAVEMANAGEMENT_OK(Slave_Management));
 475  0074 7b0a          	ld	a,(OFST+9,sp)
 476  0076 a102          	cp	a,#2
 477  0078 2709          	jreq	L47
 478  007a 7b0a          	ld	a,(OFST+9,sp)
 479  007c 2705          	jreq	L47
 480  007e ae0051        	ldw	x,#81
 481  0081 ad3f          	call	LC001
 482  0083               L47:
 483                     ; 82     assert_param(IS_SPI_CRC_POLYNOMIAL_OK(CRCPolynomial));
 485  0083 7b0b          	ld	a,(OFST+10,sp)
 486  0085 2605          	jrne	L201
 487  0087 ae0052        	ldw	x,#82
 488  008a ad36          	call	LC001
 489  008c               L201:
 490                     ; 85     SPI->CR1 = (uint8_t)((uint8_t)((uint8_t)FirstBit | BaudRatePrescaler) |
 490                     ; 86                     (uint8_t)((uint8_t)ClockPolarity | ClockPhase));
 ...
 830  0058 81            	ret	
 864                     ; 159 void SPI_SendData(uint8_t Data)
 864                     ; 160 {
 865                     .text:	section	.text,new
 866  0000               _SPI_SendData:
 870                     ; 161     SPI->DR = Data; /* Write in the DR register the data to be sent*/
 872  0000 c75204        	ld	20996,a
 873                     ; 162 }
 876  0003 81            	ret	
 899                     ; 169 uint8_t SPI_ReceiveData(void)
 899                     ; 170 {
 900                     .text:	section	.text,new
 901  0000               _SPI_ReceiveData:
 905                     ; 171     return ((uint8_t)SPI->DR); /* Return the data in the DR register*/
 907  0000 c65204        	ld	a,20996
 910  0003 81            	ret	
 947                     ; 180 void SPI_NSSInternalSoftwareCmd(FunctionalState NewState)
 947                     ; 181 {
 948                     .text:	section	.text,new
 949  0000               _SPI_NSSInternalSoftwareCmd:
 951  0000 88            	push	a
 952       00000000      OFST:	set	0
 955                     ; 183     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
 957  0001 4d            	tnz	a
 958  0002 2711          	jreq	L061
 959  0004 4a            	dec	a
 960  0005 270e          	jreq	L061
 961  0007 ae00b7        	ldw	x,#183
 962  000a 89            	pushw	x
 963  000b 5f            	clrw	x
 964  000c 89            	pushw	x
 ...
 969  0015               L061:
 970                     ; 185     if (NewState != DISABLE)
 972  0015 7b01          	ld	a,(OFST+1,sp)
 973  0017 2706          	jreq	L363
 974                     ; 187         SPI->CR2 |= SPI_CR2_SSI; /* Set NSS pin internally by software*/
 976  0019 72105201      	bset	20993,#0
 978  001d 2004          	jra	L563
 979  001f               L363:
 980                     ; 191         SPI->CR2 &= (uint8_t)(~SPI_CR2_SSI); /* Reset NSS pin internally by software*/
 982  001f 72115201      	bres	20993,#0
 983  0023               L563:
 984                     ; 193 }
 987  0023 84            	pop	a
 988  0024 81            	ret	
1011                     ; 200 void SPI_TransmitCRC(void)
1011                     ; 201 {
1012                     .text:	section	.text,new
1013  0000               _SPI_TransmitCRC:
1017                     ; 202     SPI->CR2 |= SPI_CR2_CRCNEXT; /* Enable the CRC transmission*/
1019  0000 72185201      	bset	20993,#4
1020                     ; 203 }
1023  0004 81            	ret	
1060                     ; 211 void SPI_CalculateCRCCmd(FunctionalState NewState)
1060                     ; 212 {
1061                     .text:	section	.text,new
1062  0000               _SPI_CalculateCRCCmd:
1064  0000 88            	push	a
1065       00000000      OFST:	set	0
1068                     ; 214     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
1070  0001 4d            	tnz	a
1071  0002 2711          	jreq	L471
1072  0004 4a            	dec	a
1073  0005 270e          	jreq	L471
1074  0007 ae00d6        	ldw	x,#214
...
1082  0015               L471:
1083                     ; 216     if (NewState != DISABLE)
1085  0015 7b01          	ld	a,(OFST+1,sp)
1086  0017 2706          	jreq	L514
1087                     ; 218         SPI->CR2 |= SPI_CR2_CRCEN; /* Enable the CRC calculation*/
1089  0019 721a5201      	bset	20993,#5
1091  001d 2004          	jra	L714
1092  001f               L514:
1093                     ; 222         SPI->CR2 &= (uint8_t)(~SPI_CR2_CRCEN); /* Disable the CRC calculation*/
1095  001f 721b5201      	bres	20993,#5
1096  0023               L714:
1097                     ; 224 }
1100  0023 84            	pop	a
1101  0024 81            	ret	
1166                     ; 231 uint8_t SPI_GetCRC(SPI_CRC_TypeDef SPI_CRC)
1166                     ; 232 {
...
1175                     ; 233     uint8_t crcreg = 0;
1177                     ; 236     assert_param(IS_SPI_CRC_OK(SPI_CRC));
1179  0002 a101          	cp	a,#1
1180  0004 2711          	jreq	L602
1181  0006 4d            	tnz	a
1182  0007 270e          	jreq	L602
1183  0009 ae00ec        	ldw	x,#236
...
1368  0015               L032:
1369                     ; 286     if (SPI_Direction != SPI_DIRECTION_RX)
1371  0015 7b01          	ld	a,(OFST+1,sp)
1372  0017 2706          	jreq	L525
1373                     ; 288         SPI->CR2 |= SPI_CR2_BDOE; /* Set the Tx only mode*/
1375  0019 721c5201      	bset	20993,#6
1377  001d 2004          	jra	L725
1378  001f               L525:
1379                     ; 292         SPI->CR2 &= (uint8_t)(~SPI_CR2_BDOE); /* Set the Rx only mode*/
1381  001f 721d5201      	bres	20993,#6
1382  0023               L725:
1383                     ; 294 }
1386  0023 84            	pop	a
1387  0024 81            	ret	
1509                     ; 304 FlagStatus SPI_GetFlagStatus(SPI_Flag_TypeDef SPI_FLAG)
1509                     ; 305 {
...
1518                     ; 306     FlagStatus status = RESET;
1520                     ; 308     assert_param(IS_SPI_FLAGS_OK(SPI_FLAG));
...
1623  0017               L452:
1624                     ; 343     SPI->SR = (uint8_t)(~SPI_FLAG);
...
1632  001d 84            	pop	a
1633  001e 81            	ret	
1716                     ; 359 ITStatus SPI_GetITStatus(SPI_IT_TypeDef SPI_IT)
1716                     ; 360 {
...
1725                     ; 361     ITStatus pendingbitstatus = RESET;
1727                     ; 362     uint8_t itpos = 0;
1729                     ; 363     uint8_t itmask1 = 0;
1731                     ; 364     uint8_t itmask2 = 0;
1733                     ; 365     uint8_t enablestatus = 0;
1735                     ; 366     assert_param(IS_SPI_GET_IT_OK(SPI_IT));
...
1757  0028               L662:
1758                     ; 368     itpos = (uint8_t)((uint8_t)1 << ((uint8_t)SPI_IT & (uint8_t)0x0F));
...
1773                     ; 371     itmask1 = (uint8_t)((uint8_t)SPI_IT >> (uint8_t)4);
1775  0039 7b03          	ld	a,(OFST+1,sp)
1776  003b 4e            	swap	a
1777  003c a40f          	and	a,#15
1778  003e 6b02          	ld	(OFST+0,sp),a
1779                     ; 373     itmask2 = (uint8_t)((uint8_t)1 << itmask1);
1781  0040 5f            	clrw	x
...
1790  004b               L672:
1791                     ; 375     enablestatus = (uint8_t)((uint8_t)SPI->SR & itmask2);
1793  004b c45203        	and	a,20995
1794  004e 6b02          	ld	(OFST+0,sp),a
1795                     ; 377     if (((SPI->ICR & itpos) != RESET) && enablestatus)
...
1803                     ; 380         pendingbitstatus = SET;
1805  005b a601          	ld	a,#1
1807  005d 2001          	jra	L376
1808  005f               L176:
1809                     ; 385         pendingbitstatus = RESET;
1811  005f 4f            	clr	a
1812  0060               L376:
1813                     ; 388     return  pendingbitstatus;
1817  0060 5b03          	addw	sp,#3
1818  0062 81            	ret	
1864                     ; 404 void SPI_ClearITPendingBit(SPI_IT_TypeDef SPI_IT)
1864                     ; 405 {
...
1873                     ; 406     uint8_t itpos = 0;
1875                     ; 407     assert_param(IS_SPI_CLEAR_IT_OK(SPI_IT));
1877  0002 a145          	cp	a,#69
1878  0004 2712          	jreq	L013
1879  0006 a134          	cp	a,#52
1880  0008 270e          	jreq	L013
1881  000a ae0197        	ldw	x,#407
...
1889  0018               L013:
1890                     ; 412     itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)(SPI_IT & (uint8_t)0xF0) >> 4));
