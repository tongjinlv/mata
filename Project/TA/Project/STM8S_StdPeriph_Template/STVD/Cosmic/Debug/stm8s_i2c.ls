  ...
  49                     ; 61 void I2C_DeInit(void)
  49                     ; 62 {
  51                     .text:	section	.text,new
  52  0000               _I2C_DeInit:
  56                     ; 63   I2C->CR1 = I2C_CR1_RESET_VALUE;
  58  0000 725f5210      	clr	21008
  59                     ; 64   I2C->CR2 = I2C_CR2_RESET_VALUE;
  61  0004 725f5211      	clr	21009
  62                     ; 65   I2C->FREQR = I2C_FREQR_RESET_VALUE;
  64  0008 725f5212      	clr	21010
  65                     ; 66   I2C->OARL = I2C_OARL_RESET_VALUE;
  67  000c 725f5213      	clr	21011
  68                     ; 67   I2C->OARH = I2C_OARH_RESET_VALUE;
  70  0010 725f5214      	clr	21012
  71                     ; 68   I2C->ITR = I2C_ITR_RESET_VALUE;
  73  0014 725f521a      	clr	21018
  74                     ; 69   I2C->CCRL = I2C_CCRL_RESET_VALUE;
  76  0018 725f521b      	clr	21019
  77                     ; 70   I2C->CCRH = I2C_CCRH_RESET_VALUE;
  79  001c 725f521c      	clr	21020
  80                     ; 71   I2C->TRISER = I2C_TRISER_RESET_VALUE;
 ...
 273                     ; 90 void I2C_Init(uint32_t OutputClockFrequencyHz, uint16_t OwnAddress, 
 273                     ; 91               I2C_DutyCycle_TypeDef I2C_DutyCycle, I2C_Ack_TypeDef Ack, 
 273                     ; 92               I2C_AddMode_TypeDef AddMode, uint8_t InputClockFrequencyMHz )
 273                     ; 93 {
 274                     .text:	section	.text,new
 275  0000               _I2C_Init:
 277  0000 5209          	subw	sp,#9
 278       00000009      OFST:	set	9
 281                     ; 94   uint16_t result = 0x0004;
 283                     ; 95   uint16_t tmpval = 0;
 285                     ; 96   uint8_t tmpccrh = 0;
 287  0002 0f07          	clr	(OFST-2,sp)
 288                     ; 99   assert_param(IS_I2C_ACK_OK(Ack));
 ...
 385                     ; 125     tmpccrh = I2C_CCRH_FS;
 387  009a a680          	ld	a,#128
 388  009c 6b07          	ld	(OFST-2,sp),a
 389                     ; 127     if (I2C_DutyCycle == I2C_DUTYCYCLE_2)
 391  009e 96            	ldw	x,sp
 392  009f 0d12          	tnz	(OFST+9,sp)
 393  00a1 261d          	jrne	L531
 394                     ; 130       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 3));
 ...
 414  00c0               L531:
 415                     ; 135       result = (uint16_t) ((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz * 25));
 ...
 459                     ; 151     I2C->TRISER = (uint8_t)tmpval;
 461  00f6 7b06          	ld	a,(OFST-3,sp)
 463  00f8 2028          	jra	L341
 464  00fa               L331:
 465                     ; 158     result = (uint16_t)((InputClockFrequencyMHz * 1000000) / (OutputClockFrequencyHz << (uint8_t)1));
 ...
 564  017b 81            	ret	
 620                     ; 196 void I2C_Cmd(FunctionalState NewState)
 620                     ; 197 {
 621                     .text:	section	.text,new
 622  0000               _I2C_Cmd:
 624  0000 88            	push	a
 625       00000000      OFST:	set	0
 628                     ; 200   assert_param(IS_FUNCTIONALSTATE_OK(NewState));
 630  0001 4d            	tnz	a
 631  0002 2711          	jreq	L001
 632  0004 4a            	dec	a
 633  0005 270e          	jreq	L001
 634  0007 ae00c8        	ldw	x,#200
 635  000a 89            	pushw	x
 636  000b 5f            	clrw	x
 637  000c 89            	pushw	x
...
1109  001a               L471:
1110                     ; 349   if (Ack == I2C_ACK_NONE)
1112  001a 7b01          	ld	a,(OFST+1,sp)
1113  001c 2606          	jrne	L153
1114                     ; 352     I2C->CR2 &= (uint8_t)(~I2C_CR2_ACK);
1116  001e 72155211      	bres	21009,#2
1118  0022 2011          	jra	L353
1119  0024               L153:
1120                     ; 357     I2C->CR2 |= I2C_CR2_ACK;
1122  0024 72145211      	bset	21009,#2
1123                     ; 359     if (Ack == I2C_ACK_CURR)
1125  0028 4a            	dec	a
1126  0029 2606          	jrne	L553
1127                     ; 362       I2C->CR2 &= (uint8_t)(~I2C_CR2_POS);
1129  002b 72175211      	bres	21009,#3
1131  002f 2004          	jra	L353
1132  0031               L553:
1133                     ; 367       I2C->CR2 |= I2C_CR2_POS;
1135  0031 72165211      	bset	21009,#3
1136  0035               L353:
1137                     ; 370 }
1140  0035 84            	pop	a
1141  0036 81            	ret	
1214                     ; 380 void I2C_ITConfig(I2C_IT_TypeDef I2C_IT, FunctionalState NewState)
1214                     ; 381 {
...
1333                     ; 409   assert_param(IS_I2C_DUTYCYCLE_OK(I2C_DutyCycle));
1335  0001 4d            	tnz	a
1336  0002 2712          	jreq	L032
1337  0004 a140          	cp	a,#64
1338  0006 270e          	jreq	L032
1339  0008 ae0199        	ldw	x,#409
...
1514  002f 81            	ret	
1548                     ; 459 void I2C_SendData(uint8_t Data)
1548                     ; 460 {
1549                     .text:	section	.text,new
1550  0000               _I2C_SendData:
1554                     ; 462   I2C->DR = Data;
1556  0000 c75216        	ld	21014,a
1557                     ; 463 }
1560  0003 81            	ret	
1785                     ; 579 ErrorStatus I2C_CheckEvent(I2C_Event_TypeDef I2C_Event)
1785                     ; 580 {
...
1794                     ; 581   __IO uint16_t lastevent = 0x00;
1796  0003 5f            	clrw	x
1797  0004 1f04          	ldw	(OFST-2,sp),x
1798                     ; 582   uint8_t flag1 = 0x00 ;
1800                     ; 583   uint8_t flag2 = 0x00;
1802                     ; 584   ErrorStatus status = ERROR;
1804                     ; 587   assert_param(IS_I2C_EVENT_OK(I2C_Event));
...
1848  0068               L662:
1849                     ; 589   if (I2C_Event == I2C_EVENT_SLAVE_ACK_FAILURE)
1851  0068 a30004        	cpw	x,#4
1852  006b 2609          	jrne	L336
1853                     ; 591     lastevent = I2C->SR2 & I2C_SR2_AF;
...
1861  0076               L336:
1862                     ; 595     flag1 = I2C->SR1;
1864  0076 c65217        	ld	a,21015
1865  0079 6b03          	ld	(OFST-3,sp),a
1866                     ; 596     flag2 = I2C->SR3;
1868  007b c65219        	ld	a,21017
1869  007e 6b06          	ld	(OFST+0,sp),a
1870                     ; 597     lastevent = ((uint16_t)((uint16_t)flag2 << (uint16_t)8) | (uint16_t)flag1);
1872  0080 5f            	clrw	x
...
1885                     ; 600   if (((uint16_t)lastevent & (uint16_t)I2C_Event) == (uint16_t)I2C_Event)
...
2007                     ; 649   return (I2C_Event_TypeDef)lastevent;
2009  0024 1e05          	ldw	x,(OFST-1,sp)
2012  0026 5b06          	addw	sp,#6
2013  0028 81            	ret	
2229                     ; 680 FlagStatus I2C_GetFlagStatus(I2C_Flag_TypeDef I2C_Flag)
2229                     ; 681 {
...
2238                     ; 682   uint8_t tempreg = 0;
2240  0002 0f02          	clr	(OFST+0,sp)
2241                     ; 683   uint8_t regindex = 0;
2243                     ; 684   FlagStatus bitstatus = RESET;
2245                     ; 687   assert_param(IS_I2C_FLAG_OK(I2C_Flag));
...
2287  0062               L203:
2288                     ; 690   regindex = (uint8_t)((uint16_t)I2C_Flag >> 8);
2290  0062 7b03          	ld	a,(OFST+1,sp)
2291  0064 6b01          	ld	(OFST-1,sp),a
2292                     ; 692   switch (regindex)
2295                     ; 709     default:
2295                     ; 710       break;
2296  0066 4a            	dec	a
...
2325                     ; 710       break;
2327  0080               L5101:
2328                     ; 714   if ((tempreg & (uint8_t)I2C_Flag ) != 0)
2330  0080 7b04          	ld	a,(OFST+2,sp)
2331  0082 1502          	bcp	a,(OFST+0,sp)
2332  0084 2704          	jreq	L7101
2333                     ; 717     bitstatus = SET;
2335  0086 a601          	ld	a,#1
2337  0088 2001          	jra	L1201
2338  008a               L7101:
2339                     ; 722     bitstatus = RESET;
2341  008a 4f            	clr	a
2342  008b               L1201:
2343                     ; 725   return bitstatus;
2347  008b 5b04          	addw	sp,#4
2348  008d 81            	ret	
2393                     ; 760 void I2C_ClearFlag(I2C_Flag_TypeDef I2C_FLAG)
2393                     ; 761 {
...
2402                     ; 762   uint16_t flagpos = 0;
2404                     ; 764   assert_param(IS_I2C_CLEAR_FLAG_OK(I2C_FLAG));
...
2425  001e               L213:
2426                     ; 767   flagpos = (uint16_t)I2C_FLAG & FLAG_Mask;
2428  001e 7b04          	ld	a,(OFST+2,sp)
2429  0020 5f            	clrw	x
2430  0021 02            	rlwa	x,a
2431  0022 1f01          	ldw	(OFST-1,sp),x
2432                     ; 769   I2C->SR2 = (uint8_t)((uint16_t)(~flagpos));
...
2441  002c 81            	ret	
2608                     ; 792 ITStatus I2C_GetITStatus(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2608                     ; 793 {
...
2617                     ; 794   ITStatus bitstatus = RESET;
2619                     ; 795   __IO uint8_t enablestatus = 0;
2621  0003 0f03          	clr	(OFST-1,sp)
2622                     ; 796   uint16_t tempregister = 0;
2624                     ; 799     assert_param(IS_I2C_ITPENDINGBIT_OK(I2C_ITPendingBit));
...
2658  004f               L423:
2659                     ; 801   tempregister = (uint8_t)( ((uint16_t)((uint16_t)I2C_ITPendingBit & ITEN_Mask)) >> 8);
...
2672                     ; 804   enablestatus = (uint8_t)(I2C->ITR & ( uint8_t)tempregister);
2674  005d c6521a        	ld	a,21018
2675  0060 1402          	and	a,(OFST-2,sp)
2676  0062 6b03          	ld	(OFST-1,sp),a
2677                     ; 806   if ((uint16_t)((uint16_t)I2C_ITPendingBit & REGISTER_Mask) == REGISTER_SR1_Index)
...
2686                     ; 809     if (((I2C->SR1 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2694                     ; 812       bitstatus = SET;
2696  007b 200b          	jp	LC008
2697                     ; 817       bitstatus = RESET;
2698  007d               L3311:
2699                     ; 823     if (((I2C->SR2 & (uint8_t)I2C_ITPendingBit) != RESET) && enablestatus)
...
2707                     ; 826       bitstatus = SET;
2709  0088               LC008:
2711  0088 a601          	ld	a,#1
2713  008a 2001          	jra	L1411
2714  008c               L3411:
2715                     ; 831       bitstatus = RESET;
2718  008c 4f            	clr	a
2719  008d               L1411:
2720                     ; 835   return  bitstatus;
2724  008d 5b06          	addw	sp,#6
2725  008f 81            	ret	
2771                     ; 873 void I2C_ClearITPendingBit(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
2771                     ; 874 {
...
2780                     ; 875   uint16_t flagpos = 0;
2782                     ; 878   assert_param(IS_I2C_CLEAR_ITPENDINGBIT_OK(I2C_ITPendingBit));
...
2802  0029               L633:
2803                     ; 881   flagpos = (uint16_t)I2C_ITPendingBit & FLAG_Mask;
2805  0029 7b04          	ld	a,(OFST+2,sp)
2806  002b 5f            	clrw	x
2807  002c 02            	rlwa	x,a
2808  002d 1f01          	ldw	(OFST-1,sp),x
2809                     ; 884   I2C->SR2 = (uint8_t)((uint16_t)~flagpos);
